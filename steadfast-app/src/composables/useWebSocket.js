...
export const connectWebSocket = () => {
  ...
  const getWebSocketUrl = (broker) => {
    if (isDev) {
      if (broker === 'Flattrade') return `${wsBaseUrl}:8765/ws`
      if (broker === 'Shoonya') return `${wsBaseUrl}:8766/ws`
      if (broker === 'KotakNeo') return `${wsBaseUrl}:8767/ws`
      return `${wsBaseUrl}/ws`
    } else {
  ...
  if (
    !['Flattrade', 'Shoonya', 'KotakNeo'].includes(selectedBroker.value?.brokerName) ||
...
export const subscribeToOptions = () => {
  if (socket.value && socket.value.readyState === WebSocket.OPEN) {
    ...
    if (defaultCallSecurityId.value && defaultCallSecurityId.value !== 'N/A') {
      ...
      if (currentSubscriptions.value.callOption !== defaultCallSecurityId.value) {
        ...
        currentSubscriptions.value.callOptionExchangeCode = exchangeSegment
    ...
    if (defaultPutSecurityId.value && defaultPutSecurityId.value !== 'N/A') {
      // Avoid redundant subscriptions
      if (currentSubscriptions.value.putOption !== defaultPutSecurityId.value) {
        ...
        currentSubscriptions.value.putOptionExchangeCode = exchangeSegment
