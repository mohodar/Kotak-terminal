<template>
    ...
    <form @submit.prevent>
      <fieldset :disabled="isFormDisabled" :class="{ 'disabled-form': isFormDisabled }">
        <div class="row">
          ...
          <!-- Seller Mode & Offset Combined -->
          <div class="d-none d-md-block col-6 col-md-5 col-lg-3 mt-md-1 mt-lg-2">
            <div class="d-flex align-items-end gap-2">
              <div>
                <label for="sellerMode" class="form-label mb-0 small">Seller Mode</label>
                <div class="form-check form-switch">
                  <input class="form-check-input" type="checkbox" id="sellerMode" v-model="sellerMode"
                    :class="{ 'disabled-form': isFormDisabled }" />
                  <label class="form-check-label" for="sellerMode">
                    {{ sellerMode ? 'Enabled' : 'Disabled' }}
                  </label>
                </div>
              </div>

              <div v-if="sellerMode" class="d-flex gap-2">
                <div>
                  <label for="sellerModeOffset" class="form-label mb-0 small">Hedge Offset</label>
                  <div class="input-group input-group-sm">
                    <input type="number" class="form-control form-control-sm" id="sellerModeOffset"
                      aria-label="Hedge Offset" v-model="sellerModeOffset" style="width: 80px;" min="0"
                      @input="updateHedgeOffset" @wheel="handleFormInputMouseScroll($event, { type: 'hedge_offset' })"
                      :class="{ 'disabled-form': isFormDisabled }" />
                  </div>
                </div>
                <div>
                  <label for="sellerModeExtraLots" class="form-label mb-0 small">Hedge Extra Lots</label>
                  <div class="input-group input-group-sm">
                    <input type="number" class="form-control form-control-sm" id="sellerModeExtraLots"
                      aria-label="Hedge Extra Lots" v-model="sellerModeExtraLots" style="width: 80px;" min="0"
                      @input="updateHedgeExtraLots" @wheel="handleFormInputMouseScroll($event, { type: 'hedge_extra' })"
                      :class="{ 'disabled-form': isFormDisabled }" />
                  </div>
                </div>
              </div>
        ...
        <div class="row align-items-center justify-content-between">
          ...
          <div class="order-first order-md-0 col-12 col-md-4 col-lg-4 text-center mt-3 mt-md-0">
        ...
        <div class="row align-items-center justify-content-between mt-3">
          ...
          <div class="order-1 order-md-1 order-lg-1 col-6 col-md-3 col-lg-3">
            <div class="col-12 col-md-10 col-lg-10">
              <button type="button" class="btn btn-sm btn-success fs-6 my-2 w-100"
                ...
                :data-bs-toggle="['LMT', 'SL_LMT'].includes(selectedOrderType) ? 'modal' : null"
                :data-bs-target="['LMT', 'SL_LMT'].includes(selectedOrderType) ? '#PlaceLimitOrderWindow' : null">
          ...
          <!-- Order Type -->
          <!-- Order Type -->
          <div class="order-3 order-md-2 order-lg-2 col-6 col-md-2 col-lg-2 text-center">
          ...
          <!-- Close & Cancel Buttons -->
          <!-- Close & Cancel Buttons -->
          <div class="order-5 order-md-3 order-lg-3 col-12 col-md-2 col-lg-2 text-center">
            <div class="row mt-2">
              <div class="col-12">
                <button v-if="selectedShoonyaPositionsSet.size === 0 && selectedFlattradePositionsSet.size === 0 && selectedKotakNeoPositionsSet.size === 0"
          ...
          <!-- Put Strike Buy/Sell Buttons -->
          <!-- Put Strike Buy/Sell Buttons -->
          <div class="order-2 order-md-5 order-lg-5 col-6 col-md-3 col-lg-3 text-end">
          ...
          <div class="TabContent bg-color">
            ...
            <div v-show="activeTab === 'positions'">
              ...
              <PositionsTableComponent v-if="activeFetchFunction === 'fetchFlattradePositions'"
                ...
                @place-sl-limit="placeSLLimitFromPosition"
                @modify-order="openModifyOrderModal" @cancel-order="cancelOrder" />
              ...
              <!-- KotakNeo Positions -->
              <PositionsTableComponent v-if="activeFetchFunction === 'fetchKotakNeoPositions'"
                :positions="kotakNeoPositionBook" :selected-broker="selectedBroker"
                :orders="kotakNeoOrderBook"
                :selected-positions-set="selectedKotakNeoPositionsSet"
                @update:selected-positions-set="selectedKotakNeoPositionsSet = $event"
                @set-stoploss="(position, type) => setStoploss(position, type)" @remove-stoploss="removeStoploss"
                @increase-stoploss="increaseStoploss" @decrease-stoploss="decreaseStoploss" @set-target="setTarget"
                @remove-target="removeTarget" @increase-target="increaseTarget" @decrease-target="decreaseTarget"
                @place-sl-limit="placeSLLimitFromPosition"
            ...
            <div v-show="activeTab === 'trades'">
              <OrdersNTradesComponent
                v-if="activeFetchFunction === 'fetchFlattradeOrdersTradesBook' || activeFetchFunction === 'fetchShoonyaOrdersTradesBook' || activeFetchFunction === 'fetchKotakNeoOrdersTradesBook'"
      ...
      <div class="modal-content">
        ...
        <form @submit.prevent>
          <div class="modal-body">
            <div class="col-12" v-if="selectedOrderType !== 'SL_LMT'">
              ...
              <input type="number" id="LimitPrice" class="form-control" v-model="limitPrice"
                placeholder="Enter limit price" :class="{ 'is-invalid': !isValidLimitPrice }" min="1" step="0.05" />
              <div class="invalid-feedback" v-if="!isValidLimitPrice">
                {{ limitPriceErrorMessage }}
              </div>
            </div>
            <div class="col-12 mt-2" v-if="selectedOrderType === 'SL_LMT'">
              <label for="TriggerPrice_1" class="form-label mb-0">Trigger Price</label>
              <input type="number" id="TriggerPrice_1" class="form-control" v-model="triggerPrice"
                placeholder="Enter trigger price" :class="{ 'is-invalid': !isValidLimitPrice && !triggerPrice }" min="1" step="0.05" />
            </div>
  ...
  <div class="modal fade" id="PlaceLimitOrderWithResetOrderTypeWindow" tabindex="-1"
    aria-labelledby="PlaceLimitOrderWithResetOrderTypeWindowLabel">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
        ...
        <form @submit.prevent>
          <div class="modal-body">
            ...
            <div class="col-12 mt-2" v-if="selectedOrderType === 'SL_LMT'">
              <label for="TriggerPrice_2" class="form-label mb-0">Trigger Price</label>
              <input type="number" id="TriggerPrice_2" class="form-control" v-model="triggerPrice"
                placeholder="Enter trigger price" :class="{ 'is-invalid': !isValidLimitPrice && !triggerPrice }" min="1" step="0.05" />
            </div>
  ...
  <!-- Modify Order Modal -->
  <div class="modal fade" id="modifyOrderModal" tabindex="-1" aria-labelledby="modifyOrderModalLabel" aria-hidden="true" data-bs-theme="dark">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content text-bg-dark border-secondary">
        <div class="modal-header border-secondary">
          <h5 class="modal-title fs-6" id="modifyOrderModalLabel">Modify Order: {{ currentModifyingOrder?.tsym }}</h5>
          <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <div class="mb-3">
            <label class="form-label mb-0 small text-secondary">Order ID</label>
            <input type="text" class="form-control form-control-sm bg-dark text-white border-secondary" :value="currentModifyingOrder?.norenordno" disabled>
          </div>
          <div class="row">
            <div class="col-6 mb-3">
              <label class="form-label mb-0 small text-secondary">Quantity</label>
              <input type="number" class="form-control form-control-sm bg-dark text-white border-secondary" v-model="modifyingQty">
            </div>
            <div class="col-6 mb-3">
              <label class="form-label mb-0 small text-secondary">Price</label>
              <input type="number" step="0.05" class="form-control form-control-sm bg-dark text-white border-secondary" v-model="modifyingPrice">
            </div>
          </div>
          <div class="mb-3" v-if="currentModifyingOrder?.prctyp === 'SL' || currentModifyingOrder?.prctyp === 'SL-LMT' || currentModifyingOrder?.priceType === 'SL' || currentModifyingOrder?.priceType === 'SL-LMT'">
            <label class="form-label mb-0 small text-secondary">Trigger Price</label>
            <input type="number" step="0.05" class="form-control form-control-sm bg-dark text-white border-secondary" v-model="modifyingTriggerPrice">
          </div>
        </div>
        <div class="modal-footer border-secondary">
          <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-dismiss="modal">Cancel</button>
          <button type="button" class="btn btn-sm btn-primary" @click="updateOrder" data-bs-dismiss="modal">Update Order</button>
        </div>
      </div>
    </div>
  </div>
...
// Global State
import {
  killSwitchActive, selectedCallStrike, selectedPutStrike, selectedMasterSymbol, shoonyaPositionBook, flatTradePositionBook, kotakNeoPositionBook, 
  flatOrderBook, shoonyaOrderBook, kotakNeoOrderBook,
  selectedBroker, selectedBrokerName, selectedExchange, socket, selectedProductType,
  selectedQuantity, enableStoploss, enableTarget, stoplossValue, targetValue, selectedOrderType, limitPrice, triggerPrice, selectedFlattradePositionsSet, selectedShoonyaPositionsSet, selectedKotakNeoPositionsSet, enableHotKeys, sellerMode, sellerModeOffset, sellerModeExtraLots, exchangeSymbols, selectedExpiry, selectedStrike, callStrikes, putStrikes, allSymbolsData,
  currentTime,
  showLTPRangeBar,
  showOHLCValues,
  showStrikeDetails,
...
import { closeAllPositions, cancelPendingOrders, closeSelectedPositions, cancelOrder, modifyOrder } from '@/composables/useOrderManagement'
...
onMounted(async () => {
  ...
  // Delay automatic SL/Target checks by 5 seconds to prevent triggering on page load
  // This gives time for positions to load and prevents accidental executions from cached SL/Target values
  setTimeout(() => {
    positionCheckInterval = setInterval(checkStoplossesAndTargets, 1000);
    console.log('Automatic stop-loss and target checking started (5-second delay after page load)');
  }, 5000);
});
