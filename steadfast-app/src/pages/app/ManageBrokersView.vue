...
const {
  ...
  handleKotakNeoLogin // Kotak Neo Token Generation
} = useManageBroker()
...
onMounted(() => {
  ...
  for (const broker of supportedBrokers) {
    if (broker === 'Flattrade') {
      ...
      const kotakNeoDetails = JSON.parse(localStorage.getItem(`broker_KotakNeo_${localStorage.getItem('userId')}`) || '{}');
      if (kotakNeoDetails.apiKey) {
...
<template>
        ...
        <tbody>
          ...
          <tr v-else v-for="broker in brokerDetails" :key="broker.id">
            ...
            <td class="text-center">
              <button v-if="broker.brokerName.toLowerCase().includes('shoonya')" 
                class="btn btn-outline-primary btn-sm w-75" data-bs-toggle="modal"
                data-bs-target="#ShoonyaLogin">
                Login
              </button>
              <button v-else-if="broker.brokerName.toLowerCase().includes('flat')" 
                class="btn btn-outline-primary btn-sm w-75" @click.prevent="generateToken(broker)">
                Generate Token
              </button>
              <button v-else-if="broker.brokerName.toLowerCase().includes('kotak')" 
                class="btn btn-outline-primary btn-sm w-75" data-bs-toggle="modal"
                data-bs-target="#KotakNeoLogin">
                Login
              </button>
              <span v-else class="badge bg-secondary">No Action for {{ broker.brokerName }}</span>
            </td>
            <td>
  ...
  <!-- Kotak Neo Login -->
  <div class="modal fade" id="KotakNeoLogin" tabindex="-1" aria-labelledby="KotakNeoLoginLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <form @submit.prevent="handleKotakNeoLogin" class="">
          <div class="modal-header">
            <h1 class="modal-title fs-6" id="KotakNeoLoginLabel">
              Kotak Neo Login
            </h1>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <div class="col-12">
              <div class="mb-2">
                <label for="KotakNeoBrokerUserId" class="form-label mb-0">Broker User ID (UCC)</label>
                <input type="text" id="KotakNeoBrokerUserId" class="form-control"
                  :value="manageBrokerMaskClientId(brokerDetails.find(b => b.brokerName === 'KotakNeo')?.clientId)"
                  disabled>
              </div>

              <div class="mb-2">
                <label for="KotakNeoTOTP" class="form-label mb-0">TOTP</label>
                <input type="password" id="KotakNeoTOTP" class="form-control" v-model="KOTAKNEO_TOTP"
                  placeholder="Enter Time Based OTP">
              </div>
            </div>
          </div>
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-primary" data-bs-dismiss="modal" @click="handleKotakNeoLogin">
              Login
            </button>
          </div>
        </form>
